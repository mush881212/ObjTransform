...
def trans(inp, outp):
    ...
    vmin = vertices.min(0)
    vmax = vertices.max(0)
    # calculate obj median point
    vmed = np.median(vertices, 0)
    vmed[1] = 0.5*(vmax[1]+vmin[1])
    # align model to point "move_to" (for this example, the orgin point is between the model's foot)
    move_to = [vmed[0], vmin[1], vmed[2]]
    vertices = [i - move_to for i in vertices]
    # Rotate
    degree_x = 0
    degree_y = 270
    degree_z = 0
    R = make_rotate(math.radians(degree_x), math.radians(degree_y), math.radians(degree_z))
    vertices = [np.matmul(R, i) for i in vertices]
    # Scale
    scale = 100
    vertices = [i*scale for i in vertices]
